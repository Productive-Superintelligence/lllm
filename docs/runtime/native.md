...
The v2 protocol boundary still stays small: services and packages expose typed
...
Native `Agent` records an `AgentCallSession` with exception retries,
interrupts, LLM recalls, invoke traces, delivery state, and cost. That is the
material that was missing when native was reduced to a thin v2 primitive.
...
`NativeTacticAdapter` borrows the useful adapter pattern from Pydantic AI:
...
Native can consume a v2 protocol tactic as a prompt tool:
...
This mirrors the Pydantic AI callable-tool bridge without making native depend
on Pydantic AI.
...
Builtin proxies are restored under `lllm.runtimes.native.proxies`. Test them
without live network calls unless credentials and provider access are
