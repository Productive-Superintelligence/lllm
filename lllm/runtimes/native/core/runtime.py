...
"""
Registry — the package/resource registry for an LLLM session.

All resources live in a unified ``ResourceNode``-based store keyed by
qualified refs (``"package.section:resource_path"``).  Resolution is
namespace-aware: bare keys are resolved via the default namespace.

``Runtime`` remains as the compatibility name for the registry object.
Named registries/runtimes are supported for parallel experiments.
"""
...
class Registry:
    """
    Holds package/resource registries and shared state for an LLLM session.
    ...
    def register_service(
        ...
        if loader is not None:
            node = ResourceNode.lazy(
                name, loader, namespace=namespace, resource_type="service"
            )
        else:
            node = ResourceNode.eager(
                name, service, namespace=namespace, resource_type="service"
            )
        self.register(node, overwrite=overwrite)

    def get_service(self, path: str) -> "Service":
        return self.get(path, resource_type="service")

    # ==================================================================
    # Typed convenience — Assets
    # ==================================================================

    def get_asset(self, path: str) -> Any:
        return self.get(path, resource_type="asset")
...
# ==================================================================
# Module-level singletons and named registry/runtime store
# ==================================================================
...
def get_registry(name: Optional[str] = None) -> Registry:
...
def _select_runtime(
    *,
    ...
    return runtime or registry or get_default_runtime()
...
def load_registry(*args: Any, **kwargs: Any) -> Registry:
...
def install_package(
    ...
    *,
    ...
    if load:
        ...
        rt = _select_runtime(runtime=runtime, registry=registry)
