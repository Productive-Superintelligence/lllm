...
import requests
from jupyter_client.blocking import (
    BlockingKernelClient,  # For communicating with the kernel
)
...
class JupyterSession:
    ...
    server_process: Optional[subprocess.Popen] = field(
    ...
    kernel_manager: Optional[KernelManager] = field(
        default=None, init=False, repr=False
    )
    kernel_client: Optional[BlockingKernelClient] = field(
        default=None, init=False, repr=False
    )
    ...
    def init_session(self):
        ...
        if _cutoff_date:
            ...
            assert isinstance(_cutoff_date, dt.datetime), (
                "Cutoff date must be a datetime object"
            )
...
CALL_API = proxy.__call__"""
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "JupyterSession":
        ...
        if isinstance(_datetime, str):
            _metadata["proxy"]["cutoff_date"] = dt.datetime.strptime(
                _datetime, "%Y-%m-%d"
            )
    ...
    def _ensure_notebook_file(self, create: bool = True) -> None:
        ...
        if create and not U.pexists(self.notebook_file):
            ...
            try:
                ...
                logger.error(
                    "Error creating notebook file %s: %s", self.notebook_file, e
                )
    ...
    def _write_cell(
    ...
    ) -> int:  # return the index of the cell
        self._ensure_notebook_file(create=ensure_exists)
        if not self.notebook_file:
            raise ValueError(
                "Error: Cannot write cell, notebook file not set or couldn't be created."
            )

        assert overwrite_index is None or insert_index is None, (
            "Cannot specify both overwrite_index and insert_index"
        )
        ...
        if not nb:
            ...
            )  # Should be handled by _read_notebook_object but defensive
        ...
        if overwrite_index is not None:
            if 0 <= overwrite_index < len(nb.cells):
                ...
                raise ValueError(
                    f"Error: overwrite_index {overwrite_index} out of bounds for {len(nb.cells)} cells."
                )
            ...
            if 0 <= insert_index <= len(nb.cells):
                ...
                raise ValueError(
                    f"Error: insert_index {insert_index} out of bounds for {len(nb.cells)} cells."
                )
        else:
    ...
    def delete_cells(self, index: int | List[int]):
        ...
        for i in index:
            if not nb or not (0 <= i < len(nb.cells)):
                raise ValueError(
                    f"Error: Cannot delete cell at index {i}. Notebook/cell not found or index out of bounds."
                )
    ...
    def launch_server(self, specific_port: Optional[int] = None) -> Optional[str]:
        if self.server_process and self.server_process.poll() is None:
            logger.debug(
                "Server for session '%s' already running at %s",
                self.name,
                self.server_url,
            )
        ...
        logger.debug(
            "Launching Jupyter server for session '%s' in '%s'", self.name, self.dir
        )
        try:
            ...
            url_pattern = re.compile(
                r"(http://(127\.0\.0\.1|localhost):(\d+)/\?token=\w+)"
            )
                ...
                if self.server_process.stderr:
                    ...
                    if line:
                        ...
                        if match:
                            ...
                            if port_match:
                                self.server_port = int(port_match.group(1))
                            logger.info(
                                "Jupyter server started: %s (PID %s)",
                                self.server_url,
                        ...
                        logger.error(
                            "Jupyter server process terminated unexpectedly (exit code %s)",
            ...
            logger.error(
                "Could not find Jupyter server URL in stderr within %ds",
                timeout_seconds,
            )
            ...
            logger.error(
                "Unexpected error during Jupyter server launch: %s", e, exc_info=True
            )
    ...
    def shutdown_server(self):
        if self.server_process and self.server_process.poll() is None:
            logger.info(
                "Shutting down Jupyter server for session '%s' (PID %s)",
                self.name,
                self.server_process.pid,
            )
            try:
        ...
        try:
            with lock:
                self.kernel_manager = KernelManager(
                    kernel_name="python3", env=os.environ
                )
    ...
    def run_cell(self, index: int, timeout: int = 60) -> bool:
        ...
        while not stop_iopub_gathering_loop:
            ...
            if (
                current_loop_duration > max_iopub_loop_duration
                and max_iopub_loop_duration > 0
            ):
                break
            ...
            try:
                ...
                try:
                    shell_check = self.kernel_client.get_shell_msg(
                        timeout=0
                    )  # Non-blocking check
                    if (
                        shell_check
                        and shell_check["parent_header"].get("msg_id") == msg_id
                    ):
                        stop_iopub_gathering_loop = (
                            True  # Shell reply means execution is done
                        )
                except queue.Empty:
                    pass
                ...
                logger.warning(
                    "Error processing iopub message for cell %d: %s", index, e
                )
                stop_iopub_gathering_loop = True

        execution_successful = False
        remaining_timeout_for_shell = max(
            1, timeout - (time.monotonic() - iopub_loop_start_time)
        )

        try:
            shell_reply = self.kernel_client.get_shell_msg(
                timeout=remaining_timeout_for_shell
            )
            if shell_reply["parent_header"].get("msg_id") == msg_id:
                ...
                if status == "ok":
                    cell_to_run.execution_count = shell_reply["content"].get(
                        "execution_count"
                    )
                    execution_successful = True
                elif status == "error":
                    if not any(
                        out.output_type == "error" for out in cell_to_run.outputs
                    ):
                ...
                if not any(out.output_type == "error" for out in cell_to_run.outputs):
                    cell_to_run.outputs.append(
                        nbformat.v4.new_output(
                            output_type="error",
                            ename="ShellReplyError",
                            evalue="Mismatched shell reply ID",
                            traceback=[],
                        )
                    )
            ...
            logger.warning(
                "Timeout (%.1fs) waiting for shell reply for cell %d",
                remaining_timeout_for_shell,
                index,
            )
            if not any(out.output_type == "error" for out in cell_to_run.outputs):
                cell_to_run.outputs.append(
                    nbformat.v4.new_output(
                        output_type="error",
                        ename="TimeoutError",
                        evalue="Timeout waiting for shell reply",
                        traceback=[],
                    )
                )
        except Exception as e:
    ...
    def run_all_cells(self, stop_on_error: bool = True, restart: bool = False) -> int:
        ...
        if not self.notebook_file or not U.pexists(self.notebook_file):
            logger.warning(
                "Notebook file not found for session '%s', cannot run cells", self.name
            )
            return None
        ...
        for i, cell_data in enumerate(nb.cells):
            ...
            if cell_data.cell_type == "code":
                ...
                if not success:
                    failed_cell_idx = i
                    logger.error(
                        "Error in cell %d (stop_on_error=%s)", i, stop_on_error
                    )
                    if stop_on_error:
                        break
        overall_success = failed_cell_idx is None
        logger.debug(
            "Finished running all cells for session '%s'. Overall success: %s",
            self.name,
            overall_success,
        )
        if not overall_success:
            self.last_stop_index = (
                0  # rerun all when buggy to avoid buggy cells influence the others
            )
        else:
            self.last_stop_index = len(
                nb.cells
            )  # if no error, update it, no need to rerun the cells
        if restart:
            self.shutdown()
...
class JupyterSandbox:
    ...
    def __init__(
        self, config: Dict[str, Any], path: Optional[str] = None, verbose: bool = False
    ):
        self.project_root = config["project_root"]
        self.config = config
        self.sandbox_dir = (
            path if path else U.pjoin(U.TMP_DIR, "sandbox", config["name"])
        )
    ...
    def new_session(
        ...
        proxy_cfg["activate_proxies"] = proxy_cfg.get(
            "activate_proxies", self.config["activate_proxies"]
        )
        ...
        session_name_base = (
            name
            if name
            else dt.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            + "_"
            + uuid.uuid4().hex[:6]
        )
        ...
        logger.debug(
            "Creating new session '%s' in directory: %s", session_name, session_path
        )
        U.mkdirs(session_path)
        notebook_file = U.pjoin(session_path, f"{session_name}.ipynb")
        if U.pexists(notebook_file):
            raise Exception(
                f"Session '{session_name}' already exists in {session_path}"
            )
        sess = JupyterSession(
            name=session_name,
            dir=session_path,
    ...
    def get_session(
        ...
        metadata: Optional[Dict[str, Any]] = None,
        path: Optional[str] = None,
    ) -> Optional[JupyterSession]:  # Changed default create to False
        ...
        if U.pexists(meta_file):
            ...
            return self.new_session(
                name=session_name, metadata=metadata, path=session_path
            )
        else:
    ...
    def shutdown_session_resources(self, session_name: str):
        if session_name in self.active_sessions:
            ...
            logger.warning(
                "Session '%s' not found in active sessions for resource shutdown",
                session_name,
            )
