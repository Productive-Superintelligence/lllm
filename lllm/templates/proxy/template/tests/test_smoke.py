...
def test_project_loads_without_calling_llm():
    ...
    assert runtime.has("{{package_name}}.prompts:analyst/system")
    assert runtime.has("{{package_name}}.proxies:sample")
    assert runtime.has("{{package_name}}.configs:default")
    assert runtime.has("{{package_name}}.tactics:proxy_analyst")
    ...
    assert tactic.name == "proxy_analyst"
    assert "analyst" in tactic.agents
